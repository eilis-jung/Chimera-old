...
namespace Chimera {
	...
	namespace Interpolation {
		namespace CubicMVC {
			...
			void cubicMVCs(const vector<Vector2>& points, const Vector2 & p, vector<double>& vCoords, vector<double> &gnCoords, vector<double> &gtCoords)
			{
				vector<Point2D> poly;
				for (int i = 0; i < points.size(); i++) {
					poly.push_back(Point2D(points[i].x, points[i].y));
				}
				cubicMVCs(poly, Point2D(p.x, p.y), vCoords, gnCoords, gtCoords);
			}
