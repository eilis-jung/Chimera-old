	...
	namespace CGALWrapper {
		...
		Triangulator<ScalarType>::Triangulator(vector<std::pair<ScalarType, unsigned> > v) {
			...
			for (it = v.begin(); it != v.end(); ++it) {
				...
				boundaryVerticesWithInd.push_back(tempPoint);
				boundaryVertices.push_back(tempPoint.first);
			}
		...
		vector<MVCWithInd> Triangulator<ScalarType>::getBarycentricCoordinates(ScalarType v) {
			...
			const CGAL::Barycentric_coordinates::Query_point_location queryPointLocationType = CGAL::Barycentric_coordinates::UNSPECIFIED_LOCATION;
			...
			//}
			//return res;
			Mean_value_coordinates mean_value_coordinates(boundaryVertices.begin(), boundaryVertices.end());
			CGALPoint ve = CGALPoint(v.x, v.y);
			const Output_type result = mean_value_coordinates(ve, std::back_inserter(coordinates), queryPointLocationType, algorithmType);
			int i = 0;
			for (auto & point : coordinates) {
				res.push_back(MVCWithInd(coordinates[i], i));
				i++;
			}
			return res;
		}
