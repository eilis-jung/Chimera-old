...
namespace Chimera {
	...
	namespace Meshes {
		...
		bool TriangleMesh3D::comparePairs(pair<Vector3D, int *> a, pair<Vector3D, int *> b) {
			return a.first < b.first;
		}

		bool TriangleMesh3D::comparePairs_b(pair<Vector3D, int *> a, pair<Vector3D, int *> b) {
			return *a.second < *b.second;
		}

		bool TriangleMesh3D::uniqueVectors(Vector3D a, Vector3D b) {
			return a == b;
		}

		TriangleMesh3D::TriangleMesh3D(const vector<Vector3D> &vertices, Scalar dx) : m_points(vertices) {
		...
		TriangleMesh3D::TriangleMesh3D(const CutVoxel &cutVoxel, CutCells3D *pCutCells) {
			...
			sort(allPairs.begin(), allPairs.end(), TriangleMesh3D::comparePairs);
			sort(allPairs.begin(), allPairs.end(), TriangleMesh3D::comparePairs_b);
		...
		TriangleMesh3D::TriangleMesh3D(const vector<Vector3D> &points, const vector<triangle_t> & triangles, Scalar dx, const CutVoxel &cutVoxel) {
			m_points = points;
		...
		TriangleMesh3D::TriangleMesh3D(const vector<Vector3D> &points, const vector<triangle_t> & triangles, Scalar dx) {
			m_points = points;
			m_triangles = triangles;
		...
		void TriangleMesh3D::initializeVertexNormals(const CutVoxel &cutVoxel) {
		...
		void TriangleMesh3D::alignTriangleNormals() {
		...
		void TriangleMesh3D::initializeFaceLines(const CutVoxel &cutVoxel) {
		...
		void TriangleMesh3D::clearFaceLines() {
			m_faceLinesIndices.clear();
			m_faceLinesTotalSizes.clear();
		}
		void TriangleMesh3D::followNextPointOnFace(const map<int, vector<int>> & map, int currID, int currFaceLocationID) {
		...
		void TriangleMesh3D::removeUnusedPoints() {
		...
		bool TriangleMesh3D::isInsideMesh(const Vector3D & position) {
			for(int i = 0; i < m_triangles.size(); i++) {
				Vector3D p1 = m_points[m_triangles[i].pointsIndexes[0]];
				Vector3D p2 = m_points[m_triangles[i].pointsIndexes[1]];
				Vector3D p3 = m_points[m_triangles[i].pointsIndexes[2]];
		...
		bool TriangleMesh3D::isInsideMesh2(const Vector3D & position) {
			return signedDistanceFunction(position) <= 0 + 1e-4;
		}

		bool TriangleMesh3D::isInsideMesh3(const Vector3D & position) {
...
*/
		...
		Scalar TriangleMesh3D::signedDistanceFunction(const Vector3D & point) {
		...
		bool TriangleMesh3D::isRectangularFace(const CutFace<Vector3D> *pCutFace) {
